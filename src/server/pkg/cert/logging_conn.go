...
type loggingConn struct {
	// The loggingPipe over which this connection is communicating. Exported so
	// that callers can get communication logs from it
	pipe *loggingPipe
	r    io.Reader
	w    io.WriteCloser
}
...
type TestListener struct {
	...
	// Accept().  Exported so that tests can extract and examine communication
	// logs
	conn   *loggingConn
	connMu sync.Mutex
